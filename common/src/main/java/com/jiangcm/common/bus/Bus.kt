package com.jiangcm.common.bus
...
object Bus {
    ...
    inline fun <reified T> post(channel: String, value: T) {
        LiveEventBus[channel, T::class.java].post(value)
    }
    ...
    inline fun <reified T> observe(channel: String, owner: LifecycleOwner, observer: Observer<T>) {
        LiveEventBus[channel, T::class.java].observe(owner, observer)
    }
    ...
    inline fun <reified T> observeForever(channel: String, observer: Observer<T>) {
        LiveEventBus[channel, T::class.java].observeForever(observer)
    }
    ...
    inline fun <reified T> observeSticky(
        ...
        LiveEventBus[channel, T::class.java].observeSticky(owner, observer)
    }
    ...
    inline fun <reified T> observeStickyForever(
        ...
        LiveEventBus[channel, T::class.java].observeStickyForever(observer)
    }
}
