package com.jiangcm.base.coroutines
 ...
 * 注意：该函数会阻塞代码继续执行
 */
inline fun taskBlockOnWorkThread(delayTime: Long = 0, noinline job:suspend () -> Unit) = runBlocking(
    ThreadPool
) {
    delay(delayTime)
    job()
 ...
 * 特点带返回值
 */
inline fun <T> taskAsync(delayTime: Long = 0, noinline job:suspend () -> T) = GlobalScope.async(
    ThreadPool
) {
    delay(delayTime)
    job()
 ...
 * 特点不带返回值
 */
inline fun <T> taskLaunch(delayTime: Long = 0, noinline job:suspend  () -> T) = GlobalScope.launch(
    ThreadPool
) {
    delay(delayTime)
    job()
 ...
 */
inline fun <T> taskRunOnUiThread(noinline job:suspend  () -> T) = GlobalScope.launch(UI) {
    job()
}
